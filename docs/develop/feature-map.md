...
| 项目导入 | `projects/components/import-dialog.tsx`、`projects/lib/import-api.ts` | `api/routers/import_router.py` -> `storage/services/import_service.py` | `api/schemas/import_schema.py`；`backend/tests/api/test_import.py` | 分“预览、确认、流式确认”；格式变更要保持预览与实际导入一致 |
