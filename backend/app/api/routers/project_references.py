...
async def get_material(project_id: str, resource: service.Resource, source_id: str, session: Session):
    try:
        return await service.read_shared_material(session, project_id, source_id, resource)
    except NotFoundError as exc:
        raise HTTPException(404, str(exc)) from exc
