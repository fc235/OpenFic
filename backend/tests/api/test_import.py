...
async def test_document_import_endpoints_preserve_order_and_placement(
    ...
    from tests.core.test_document_import import _make_epub
    ...
    files = [
        ...
        (
            ...
            (
                "archive.zip",
                _build_zip([("ZIP卷/第二十章 后续.txt", "内容".encode())]),
                "application/zip",
            ),
        ),
        ("files", ("book.epub", _make_epub(), "application/epub+zip")),
    ]
    ...
    assert [volume["order"] for volume in volumes] == list(range(1, 6))
    assert [
        volume["id"]
    ...
    assert [volume["order"] for volume in appended] == list(range(1, 8))
    ...
    for limit, value, uploads, expected_detail in (
        ...
        (
            ...
            [
                (
                    "files",
                    ("one.zip", _build_zip([("one.txt", b"abc")]), "application/zip"),
                ),
                (
                    "files",
                    ("two.zip", _build_zip([("two.txt", b"abc")]), "application/zip"),
                ),
            ],
            "解压后总大小",
        ),
