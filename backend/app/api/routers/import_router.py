...
MAX_IMPORT_UNCOMPRESSED_SIZE = 100 * 1024 * 1024
...
async def _read_document_import(
    ...
    total_size = 0
    for file in files:
        if not is_supported_import_file(file.filename):
            raise HTTPException(400, "仅支持 .txt、.md、.zip 或 .epub 文件")
    ...
    try:
        ...
        for document in documents:
            if document.filename.lower().endswith((".zip", ".epub")):
                with zipfile.ZipFile(io.BytesIO(document.content)) as archive:
                    expanded_size += sum(
                        member.file_size for member in archive.infolist()
                    )
                if expanded_size > MAX_IMPORT_UNCOMPRESSED_SIZE:
                    raise ValueError("压缩包解压后总大小超过限制（最大 100MB）")
